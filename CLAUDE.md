...
- **Baskı JPEG'i başlıksız yazılmaz:** `electron/raster.ts` içindeki sharp zinciri `.withMetadata({ density: 300 })` içermek zorundadır. sharp varsayılan olarak APP bloğu yazmaz (dosya `ffd8 ffdb` ile başlar); CUPS'un `imagetoraster` filtresi böyle bir JPEG'i açamaz, **hata vermeden 0 baytlık raster** üretir ve iş `canceled-at-device` ile düşer — arayüzde hiçbir belirti olmadan hiçbir baskı çıkmaz (2026-09-20'de gerçek DS620 ile bulundu). Sorunun renk alt örneklemesiyle ilgisi yok, `4:4:4` kalabilir. Doğrulama: `cupsfilter -p /etc/cups/ppd/<yazıcı>.ppd -m application/vnd.cups-raster <dosya.jpg> | wc -c` — 0 dönerse dosyayı filtre tanımıyordur.
